...
# The same as above but different exclusion criteria
slopes_no_corr_wo_sep_infs <- list(
  linear_power = function(x) lme4::lmer(RT_with_Effect ~ 1 + SimCond + 
                                          (1 + SimCond || UniqueSubject) + 
                                          (1 + SimCond || UniqueItem), 
                                        data = filter(x, 
...
no_slopes_wo_infs_plus_logshift <- no_slopes_wo_infs %>%
  append(
    list(
      lgshift_power= function(x) lme4::lmer(log10(RT_with_Effect-PowerShift) ~ 1 + SimCond + 
                                              (1  | UniqueSubject) + 
                                              (1  | UniqueItem), 
                                            data = filter(x, 
                                                          RT_with_Effect > 0,
                                                          RT > 0), 
                                            control = lme4::lmerControl(optimizer="bobyqa")), 
      lgshift_type1= function(x) lme4::lmer(log10(RT-TypeIShift) ~ 1 + SimCond + 
                                              (1 | UniqueSubject) + 
                                              (1 | UniqueItem), 
                                            data = filter(x, 
                                                          RT_with_Effect > 0,
...
safe_residual_no_slopes_no_subject_w_logshift <- safe_residual_no_slopes_no_subject %>%
   append(
    list(
      lgshift_power = function(x) {
        if (all(is.na(x$LgShftRT_Resid_with_Effect)) | all(is.nan(x$LgShftRT_Resid_with_Effect))) {
          stop("Log power BATA residualization model did not converge")
        }
        lme4::lmer(LgShftRT_Resid_with_Effect ~ 1 + SimCond + (1  | UniqueItem), data = x,
                   control = lme4::lmerControl(optimizer="bobyqa"))
      },
      lgshift_type1 = function(x) {
        if (all(is.na(x$LgShftRT_Resid)) | all(is.nan(x$LgShftRT_Resid))) {
          stop("Log type 1 BATA residualization model did not converge")
        }
        lme4::lmer(LgShftRT_Resid ~ 1 + SimCond + (1 | UniqueItem), data = x,
                   control = lme4::lmerControl(optimizer="bobyqa"))
      }
    )
   )
