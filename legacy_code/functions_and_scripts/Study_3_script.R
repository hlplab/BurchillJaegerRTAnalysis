...
para_giant_mm_df <- tidyr::crossing(Bins = "giant",
  ...
  mutate(MaxK = map_dbl(Knum, ~max(k_list[[.]])),
         ...
         bb_file = paste0(path_on_server, "amlap_saved_files/bb_", Distribution, "_",
         ...
         mm_file = paste0(path_on_server, "amlap_saved_files/modeldata_", Distribution, "_",
                          ...
                          MinK, "_to_", MaxK, ".RDS"),
         format_file = paste0(path_on_server, "amlap_saved_files/bb_",
                              iprefix, "_", Nsubj, "_", Nitems, "_", Bins, "_",
                              MinK, "_to_", MaxK, ".RDS")) %>%
  filter(Nsubj == Nitems) %>%
  filter(Space == "") #%>% filter(Distribution =="gaussian_para")

para_giant_bb_df <- para_giant_mm_df %>%
  select(-Space, -EffectSize, -RType, -mm_file) %>% distinct()
remaining_para_giant_bb_files <- remaining_files(para_giant_bb_df, file.exists(bb_file))

# Save BB files
para_giant_bb %beep% {
  iterate_over_df(
    remaining_para_giant_bb_files,
    future_cnd_map,
    function(zaa) {
      
      # Establish Inverse functions
...
# Make MM files
remaining_para_giant_mm_files <- remaining_files(para_giant_mm_df, file.exists(mm_file))
# Saves MM files
para_giant_mm %beep% {
  iterate_over_df(
    remaining_para_giant_mm_files,
    future_cnd_map,
    function(zaa) {
...
para_distro_df <- para_giant_mm_df %>%
  ...
  mutate(mm_file = paste0(path_on_server, 
                          "amlap_saved_files/statdistro_", Distribution, "_",
                          iprefix, "_", Nsubj, "_", Nitems, "_", Bins, "_",
                          RType, "_size_", EffectSize, Space, "_",
                          MinK, "_to_", MaxK, ".RDS"))
...
para_giant_load %beep% {
  iterate_over_df(
    ...
    saveRDS(paste0(path_on_server, "amlap_saved_files/results/parametric_giant_lmerTest_new.RDS"))
  "rsync -avzh zburchil@cycle1.cs.rochester.edu:/u/zburchil/workspace/launchpad/amlap_saved_files/results/parametric_giant_lmerTest_new.RDS ~/Box\ Sync/Power\ simulations\ for\ RTs/PowerSimulations/data/powerpaper/"
} %plan% elite_squad_plan
done(para_giant_load)
