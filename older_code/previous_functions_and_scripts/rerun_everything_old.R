...
# simple_models_test <- list(
#   ourwaypower = function(x) lm(ourWayEffect ~ 1 + SimCond, data = x),
#   ourwaytype1 = function(x) lm(ourWayNoEffect ~ 1 + SimCond, data = x),
#   theirwaypower = function(x) lm(theirWayEffect ~ 1 + SimCond, data = x),
#   theirwaytype1 = function(x) lm(theirWayNoEffect ~ 1 + SimCond, data = x)
# )
# 
...
# 
# # Testing removing slopes
# no_item_slopes_mixed_model_func_list <- list(
#   ourwaypower = function(x)     lme4::lmer(ourWayEffect ~ 1 + SimCond + (1 + SimCond | UniqueSubject) + (1 | UniqueItem), data = x,  control = lme4::lmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 1000000))),
#   ourwaytype1 = function(x)     lme4::lmer(ourWayNoEffect ~ 1 + SimCond + (1 + SimCond | UniqueSubject) + (1 | UniqueItem), data = x,  control = lme4::lmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 1000000))),
#   theirwaypower = function(x)   lme4::lmer(theirWayEffect ~ 1 + SimCond + (1 + SimCond | UniqueSubject) + (1 | UniqueItem), data = x,  control = lme4::lmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 1000000))),
#   theirwaytype1 = function(x) lme4::lmer(theirWayNoEffect ~ 1 + SimCond + (1 + SimCond | UniqueSubject) + (1 | UniqueItem), data = x,  control = lme4::lmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 1000000)))
# )
...
good_bbs %beep% {
  iterate_over_df(
    ...
    function(i) {
      ...
      } else if (DataSet %in% c("fetal", "setal_small")) {
...
single_file %beep% {
  iterate_over_df(
    ...
    saveRDS(paste0(saver_path, "beyond_simple_effect_results.RDS"))
  "done!"
}
done(single_file)
